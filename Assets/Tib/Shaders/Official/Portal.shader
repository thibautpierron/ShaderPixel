...
Shader "PixelShader/Portal"
{
	Properties
	{
		...
		_Light("Light", vector) = (0.0, 0.0, 0.0, 0.0)
	}
	SubShader
	{
		...
		Pass
		{
			...
			float smoothStep( in float2 p ) {
				...
				return -1.0 + 2.0 * lerp( 
							lerp(noise(i + float2(0.0, 0.0)), noise(i + float2(1.0, 0.0)), u.x),
			...
			float map(float3 p, int detailLevel) {
				float freq = _SeaFreq;
			...
			float3 getSeaColor(float3 p, float3 n, float3 l, float3 eye, float3 dist) {  
				...
				color += _SeaColor * (p.y - _SeaHeight) * 0.18 * atten;
